...
from hypothesis.stateful_asyncio import AsyncioRuleBasedStateMachine
...
from tests.nocover.test_stateful_async import DepthMachine

NO_BLOB_SETTINGS = Settings(print_blob=False, phases=tuple(Phase)[:-1])


class MixedSyncAndAsyncMachine(AsyncioRuleBasedStateMachine):
    @rule()
    def sync_rule(self):
        pass

    @rule()
    async def async_rule(self):
        pass


TestMixedSyncAndAsyncMachine= MixedSyncAndAsyncMachine.TestCase
TestMixedSyncAndAsyncMachine.settings = Settings(TestMixedSyncAndAsyncMachine.settings, max_examples=10)


class MultipleRulesSameFuncMachine(AsyncioRuleBasedStateMachine):
    async def myfunc(self, data):
        print(data)
...
class PreconditionMachine(AsyncioRuleBasedStateMachine):
    num = 0
    ...
    async def div_by_precondition_after(self, num):
        self.num = num / self.num

    @precondition(lambda self: self.num != 0)
    @rule(num=integers())
    async def div_by_precondition_before(self, num):
        self.num = num / self.num
...
class FlakyDrawLessMachine(AsyncioRuleBasedStateMachine):
    @rule(d=data())
    async def action(self, d):
...
def test_result_is_added_to_target():
    class TargetStateMachine(AsyncioRuleBasedStateMachine):
        nodes = Bundle("nodes")

        @rule(target=nodes, source=lists(nodes))
        async def bunch(self, source):
            assert len(source) == 0
            return source
...
class FlakyStateMachine(AsyncioRuleBasedStateMachine):
    @rule()
...
class FlakyRatchettingMachine(AsyncioRuleBasedStateMachine):
    ratchet = 0
...
class MachineWithConsumingRule(AsyncioRuleBasedStateMachine):
    b1 = Bundle("b1")
    b2 = Bundle("b2")
...
class MachineUsingMultiple(AsyncioRuleBasedStateMachine):
    b = Bundle("b")
    ...
    async def populate_bundle(self, items):
        self.expected_bundle_length += len(items)
        return multiple(*items)

    @rule(target=b)
    async def do_not_populate(self):
        return multiple()
...
def test_multiple_variables_printed():
    class ProducesMultiple(AsyncioRuleBasedStateMachine):
        b = Bundle("b")

        @initialize(target=b)
        async def populate_bundle(self):
            return multiple(1, 2)

        @rule()
        async def fail_fast(self):
            raise AssertionError
    ...
    b_0, b_1 = asyncio.run(state.populate_bundle())
    with raises(AssertionError):
        asyncio.run(state.fail_fast())
...
def test_multiple_variables_printed_single_element():
    ...
    state = ProducesMultiple()
    (v1,) = asyncio.run(state.populate_bundle())
    asyncio.run(state.fail_fast((v1,)))  # passes if tuple not unpacked
    with raises(AssertionError):
        asyncio.run(state.fail_fast(v1))


def test_no_variables_printed():
    class ProducesNoVariables(AsyncioRuleBasedStateMachine):
        b = Bundle("b")
...
def test_empty_machine_is_invalid():
    class EmptyMachine(AsyncioRuleBasedStateMachine):
        pass
...
def test_machine_with_no_terminals_is_invalid():
    class NonTerminalMachine(AsyncioRuleBasedStateMachine):
        @rule(value=Bundle("hi"))
        async def bye(self, hi):
            pass
...
def test_minimizes_errors_in_teardown():
    ...
    class Foo(AsyncioRuleBasedStateMachine):
        @initialize()
        async def init(self):
            nonlocal counter
            counter = 0
...
class RequiresInit(AsyncioRuleBasedStateMachine):
    def __init__(self, threshold):
        super().__init__()
        self.threshold = threshold

    @rule(value=integers())
    async def action(self, value):
        if value > self.threshold:
            raise ValueError(f"{value} is too high")


def test_can_use_factory_for_tests():
    machine = RequiresInit(42)
    try:
        with raises(ValueError):
            run_state_machine_as_test(
                lambda: machine, settings=Settings(max_examples=100)
            )
    finally:
        machine.teardown()


class FailsEventually(AsyncioRuleBasedStateMachine):
    def __init__(self):
        super().__init__()
        self.counter = 0
...
def test_stateful_double_rule_is_forbidden(recwarn):
    with pytest.raises(InvalidDefinition):
        ...
        class DoubleRuleMachine(AsyncioRuleBasedStateMachine):
            @rule(num=just(1))
            @rule(num=just(2))
            async def whatevs(self, num):
                pass


def test_can_explicitly_call_functions_when_precondition_not_satisfied():
    class BadPrecondition(AsyncioRuleBasedStateMachine):
        def __init__(self):
            super().__init__()
...
def test_invariant():
    ...
    class Invariant(AsyncioRuleBasedStateMachine):
        def __init__(self):
            super().__init__()
...
def test_invalid_rule_argument():
    ...
    with pytest.raises(InvalidArgument):
        ...
        class InvalidRuleMachine(AsyncioRuleBasedStateMachine):
...
def test_invalid_initialize_argument():
    ...
    with pytest.raises(InvalidArgument):
        ...
        class InvalidInitialize(AsyncioRuleBasedStateMachine):
...
def test_invariant_failling_present_in_falsifying_example():
    ...
    class BadInvariant(AsyncioRuleBasedStateMachine):
        @initialize()
...
def test_invariant_present_in_falsifying_example():
    ...
    class BadRuleWithGoodInvariants(AsyncioRuleBasedStateMachine):
        def __init__(self):
            super().__init__()
            self.num = 0
...
def test_always_runs_at_least_one_step():
    class CountSteps(AsyncioRuleBasedStateMachine):
        def __init__(self):
            super().__init__()
            self.count = 0
...
def test_removes_needless_steps():
    ...
    class IncorrectDeletion(AsyncioRuleBasedStateMachine):
        def __init__(self):
            super().__init__()
...
def test_prints_equal_values_with_correct_variable_name():
    ...
    class MovesBetweenBundles(AsyncioRuleBasedStateMachine):
        b1 = Bundle("b1")
        b2 = Bundle("b2")
...
def test_initialize_rule():
    ...
    class WithInitializeRules(AsyncioRuleBasedStateMachine):
...
def test_initialize_rule_populate_bundle():
    class WithInitializeBundleRules(AsyncioRuleBasedStateMachine):
        a = Bundle("a")
...
def test_initialize_rule_dont_mix_with_precondition():
    with pytest.raises(InvalidDefinition) as exc:
        ...
        class BadStateMachine(AsyncioRuleBasedStateMachine):
    ...
    with pytest.raises(InvalidDefinition) as exc:
        ...
        class BadStateMachineReverseOrder(AsyncioRuleBasedStateMachine):
            @initialize()
            @precondition(lambda self: True)
            def initialize(self):
                pass
...
def test_initialize_rule_in_state_machine_with_inheritance():
    class ParentStateMachine(AsyncioRuleBasedStateMachine):
        initialized: ClassVar = []
...
def test_can_manually_call_initialize_rule():
    class StateMachine(AsyncioRuleBasedStateMachine):
...
def test_steps_printed_despite_pytest_fail():
    ...
    @Settings(print_blob=False)
    class RaisesProblem(AsyncioRuleBasedStateMachine):
        @rule()
        def oops(self):
...
def test_uses_seed(capsys):
    @seed(0)
    class TrivialMachine(AsyncioRuleBasedStateMachine):
        @rule()
        def oops(self):
            raise AssertionError
...
def test_arguments_do_not_use_names_of_return_values():
    ...
    class TrickyPrintingMachine(AsyncioRuleBasedStateMachine):
        data = Bundle("data")
...
class TrickyInitMachine(AsyncioRuleBasedStateMachine):
    @initialize()
    def init_a(self):
        self.a = 0
...
class MinStepsMachine(AsyncioRuleBasedStateMachine):
    @initialize()
    def init_a(self):
        self.a = 0
...
class ErrorsOnClassAttributeSettings(AsyncioRuleBasedStateMachine):
...
def test_single_target_multiple():
    class Machine(AsyncioRuleBasedStateMachine):
        a = Bundle("a")
...
class LotsOfEntropyPerStepMachine(AsyncioRuleBasedStateMachine):
