...
class BalancedTrees(AsyncioRuleBasedStateMachine):
    trees = Bundle("BinaryTree")

    @rule(target=trees, x=st.booleans())
    async def leaf(self, x):
        return Leaf(x)

    @rule(target=trees, left=trees, right=trees)
    async def split(self, left, right):
        return Split(left, right)

    @rule(tree=trees)
    async def test_is_balanced(self, tree):
        if isinstance(tree, Leaf):
            return
        else:
            assert abs(self.size(tree.left) - self.size(tree.right)) <= 1
            await self.test_is_balanced(tree.left)
            await self.test_is_balanced(tree.right)
...
class DepthMachine(AsyncioRuleBasedStateMachine):
    charges = Bundle("charges")

    @rule(targets=(charges,), child=charges)
    async def charge(self, child):
        return DepthCharge(child)

    @rule(targets=(charges,))
    async def none_charge(self):
        return DepthCharge(None)

    @rule(check=charges)
    async def is_not_too_deep(self, check):
        assert check.depth < 3


class RoseTreeStateMachine(AsyncioRuleBasedStateMachine):
    nodes = Bundle("nodes")
...
class NotTheLastMachine(AsyncioRuleBasedStateMachine):
    stuff = Bundle("stuff")
    ...
    @rule(v=stuff)
    async def bye(self, v):
        assert v == self.last
        self.bye_called = True


class PopulateMultipleTargets(AsyncioRuleBasedStateMachine):
    b1 = Bundle("b1")
    b2 = Bundle("b2")
    ...
    @rule(x=b1, y=b2)
    async def fail(self, x, y):
        raise AssertionError


class CanSwarm(AsyncioRuleBasedStateMachine):
...
class MyStatefulMachine(AsyncioRuleBasedStateMachine):
    def __init__(self):
        self.n_steps = 0
        super().__init__()
...
def test_multiple_precondition_bug():
    ...
    class MultiplePreconditionMachine(AsyncioRuleBasedStateMachine):
        @rule(x=st.integers())
        async def good_method(self, x):
            pass

        @precondition(lambda self: True)
        @precondition(lambda self: False)
        @rule(x=st.integers())
        async def bad_method_a(self, x):
            raise AssertionError("This rule runs, even though it shouldn't.")

        @precondition(lambda self: False)
        @precondition(lambda self: True)
        @rule(x=st.integers())
        async def bad_method_b(self, x):
            raise AssertionError("This rule might be skipped for the wrong reason.")

        @precondition(lambda self: True)
        @rule(x=st.integers())
        @precondition(lambda self: False)
        async def bad_method_c(self, x):
            raise AssertionError("This rule runs, even though it shouldn't.")

        @rule(x=st.integers())
        @precondition(lambda self: True)
        @precondition(lambda self: False)
        async def bad_method_d(self, x):
            raise AssertionError("This rule runs, even though it shouldn't.")

        @precondition(lambda self: True)
        @precondition(lambda self: False)
        @invariant()
        async def bad_invariant_a(self):
            raise AssertionError("This invariant runs, even though it shouldn't.")

        @precondition(lambda self: False)
        @precondition(lambda self: True)
        @invariant()
        async def bad_invariant_b(self):
            raise AssertionError("This invariant runs, even though it shouldn't.")

        @precondition(lambda self: True)
        @invariant()
        @precondition(lambda self: False)
        async def bad_invariant_c(self):
            raise AssertionError("This invariant runs, even though it shouldn't.")

        @invariant()
        @precondition(lambda self: True)
        @precondition(lambda self: False)
        async def bad_invariant_d(self):
            raise AssertionError("This invariant runs, even though it shouldn't.")

    run_state_machine_as_test(MultiplePreconditionMachine)


class UnrelatedCall(AsyncioRuleBasedStateMachine):
    a = Bundle("a")
    ...
    async def unrelated(self, value):
...
class SourceSameAsTarget(AsyncioRuleBasedStateMachine):
    values = Bundle("values")

    @rule(target=values, value=st.lists(values))
    async def f(self, value):
        assert len(value) == 0
        return value


class SourceSameAsTargetUnclearOrigin(AsyncioRuleBasedStateMachine):
    values = Bundle("values")
